...
public class Square {
	...
	public boolean canRotate(Square center) {
        ...
        int newCol = center.col + (center.row - row);
        int newRow = center.row + (col - center.col);

        if ((newCol < 0 || grid.isSet(newRow, newCol - 1))  (newCol > Grid.WIDTH - 1  || grid.isSet(newRow, newCol))) {
            move = false;
        }
	...
	public void rotate(int centralRow, int centralCol) {
		int tempRow = row;
		row = (col + centralRow - centralCol);
		col = (centralRow + centralCol - tempRow);
	}
