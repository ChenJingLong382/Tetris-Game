...
public class Grid {
	...
	private void shiftRows(int deletedRow) {
		for (int row = deletedRow; row > 0; row--) {
			for (int column = 0; column < WIDTH; column++) {
				if (isOccupied(row-1, column)) {
					board[row][column].setColor(Color.magenta);
				} else {
					board[row][column].setColor(EMPTY);
				}
			}
		}
	}
